...
def get_whisper_model(model_size: str = "small.en"):
    ...
    try:
        log.info(f"[ASR_ENGINE] Attempting to load faster-whisper '{model_size}' on GPU (cuda)...")
        _WHISPER_MODELS[model_size] = WhisperModel(model_size, device="cuda", compute_type="float16")
        log.info("[ASR_ENGINE] GPU load successful.")
    except Exception as e_cuda:
        log.warning(f"[ASR_ENGINE] GPU load failed ({e_cuda}). Falling back to CPU...")
        try:
